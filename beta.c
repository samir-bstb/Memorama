...
int level = 0; // 1 easy, 3 medium, 4 hard
...
void start(){
	...
	LCD_WR_string("Push any button");
	int x = read_buttons();
	if(x == 1){
		LCD_WR_inst(LCD_clean);
		LCD_WR_string("Select level");
		LCD_WR_inst(0b11000000);
		LCD_WR_string("easy  mid  hard");
		select_level();
		HAL_Delay(1000);
		LCD_WR_inst(LCD_clean);
...
void play_sequence(int lim){
	for(int i = 0; i< lim; i++){
		...
		HAL_Delay(led_delay); //should be a parameter,and modified depending on the level
